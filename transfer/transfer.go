...
func TrainSearch(c *gin.Context) {
	...
	*/
	...
	if err := c.Bind(&req); err != nil {
		...
		client := &http.Client{}
		req, err := http.NewRequest("GET", fmt.Sprintf(baseUrl, req.StartDate, req.ArriveCty, req.StartCty), nil)
		if err != nil {
...
func FlightSearch(c *gin.Context) {
	...
	if err := c.Bind(&req); err != nil {
		...
		*/
		...
		client := &http.Client{}
		req, err := http.NewRequest("GET", fmt.Sprintf(baseUrl, req.ArriveCty, req.StartCty, req.StartDate), nil)
		if err != nil {
			fmt.Printf("get error: %v", err.Error())
		}
		req.Header.Add("Authorization", fmt.Sprintf("APPCODE %s", appCode))
